...
export class EmployeeContactComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.employeeContactService
      ...
      .subscribe({
        next: (res: HttpResponse<IEmployeeContact[]>) => {
  ...
  trackId(_index: number, item: IEmployeeContact): number {
    return item.id!;
  }

  delete(employeeContact: IEmployeeContact): void {
    const modalRef = this.modalService.open(EmployeeContactDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEmployeeContact[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/jhipstersampleapplication/employee-contact'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
