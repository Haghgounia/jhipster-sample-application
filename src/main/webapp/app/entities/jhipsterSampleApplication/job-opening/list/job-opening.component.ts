...
export class JobOpeningComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.jobOpeningService
      ...
      .subscribe({
        next: (res: HttpResponse<IJobOpening[]>) => {
  ...
  trackId(_index: number, item: IJobOpening): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IJobOpening[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/jhipstersampleapplication/job-opening'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
