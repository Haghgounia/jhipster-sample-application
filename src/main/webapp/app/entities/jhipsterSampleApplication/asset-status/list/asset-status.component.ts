...
export class AssetStatusComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.assetStatusService
      ...
      .subscribe({
        next: (res: HttpResponse<IAssetStatus[]>) => {
  ...
  trackId(_index: number, item: IAssetStatus): number {
    return item.id!;
  }

  delete(assetStatus: IAssetStatus): void {
    const modalRef = this.modalService.open(AssetStatusDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IAssetStatus[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/jhipstersampleapplication/asset-status'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
